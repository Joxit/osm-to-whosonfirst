...
module.exports = class ReadShapefile {
  constructor({shp, dbf, evaluate, fromEPSG, toEPSG}) {
    ...
    this.opts = {};
    this.opts.shp = shp;
    this.opts.dbf = dbf || this.getDBFPath();
  }
  readAsync() {
    let self = this;
  ...
  getDBFPath() {
    return this._getPathForExt('.dbf');
  }
  getProjPath() {
    return this._getPathForExt('.prj');
  }
  _getPathForExt(ext) {
    let regexp = new RegExp(ext, 'i');
    let shpPath = path.parse(this.opts.shp);
    let name = fs.readdirSync(shpPath.dir).find((f) => {
      let p = path.parse(f);
      return shpPath.name == p.name && regexp.test(p.ext)
    });
    return name ? path.join(shpPath.dir, name) : undefined;
  }
